...
2. 大容量ファイル・MXFファイルはFargate + FFmpegでProxy動画に変換（15分セグメント単位、最大24並列）
3. Geminiモデル（gemini-2.5-flash）に対し、定義済みのシステムプロンプトと動画を送信（最大5並列）
4. AIからのレスポンス（Markdownテーブル形式のテキスト）をパース
5. パースに失敗した場合、フォーマット修正用のGemini（gemini-2.5-flash）を使用して出力形式を修正し、再度パースを試行（最大3回）
...
| Infrastructure | AWS Lambda + API Gateway + ECS Fargate | サーバーレス、スケーラブル、長時間処理対応 |
